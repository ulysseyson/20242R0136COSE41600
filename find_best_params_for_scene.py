...
for p in ps[:]:
    ...
    max_height = 1.5  # Z값 차이의 최대값

    max_distance = 400.0  # 원점으로부터의 최대 거리
    # ========================
    # use just middle 10 frames for test
    print(len(file_paths) // 2 - 5, ":", len(file_paths) // 2 + 5)
    file_paths = file_paths[len(file_paths) // 2 - 10 : len(file_paths) // 2 + 10]
    for file_path in tqdm(file_paths):
        ...
        for i in range(max_label + 1):
            ...
            # add cluster points to vis
            vis.add_geometry(final_point.select_by_index(cluster_indices))
            if list(cluster_indices) == [
                1308,
                1344,
                1777,
                2303,
        ...
        vis.update_renderer()
        i = 0
        os.makedirs(f"tmp/param_grid{i}/{p}", exist_ok=True)
        vis.capture_screen_image(f"tmp/param_grid{i}/{p}/{file_path}.png")
        time.sleep(1)
        # vis.run()
        # vis.close()
    vis.close()
